...
class Attending extends ControllerBase {
  ...
  public function build(): array {
    ...
    // Only vendors this user has permission to view.
    ->accessCheck()
    ...
    if (count($node_ids) > 0) {
      ...
      // We're going to display each vendor twice. Once in an unordered list that we'll use for a summary at the top of the page. And then again using the configured 'teaser' view mode below that list. This allows us to demonstrate both rendering individual fields and complete entities.
      $vendor_list = [];
      $vendor_teasers = [];
      ...
      foreach ($nodes as $vendor) {
        ...
        // And, the email address from the field_vendor_contact_email field rendered using the configured formatter for hte 'default' view mode. But we're also going to explicitly hide the field label regardless of what's configured for the view mode.
        // See \Drupal\Core\Field\FieldItemListInterface::view()
        // Calling view() on the field is a wrapper using the ViewBuilder like so: $view_builder->viewField($vendor->field_vendor_contact_email);
      ...
      // Alternatively, we could render teasers for all vendors at once using $vendor_teasers = $view_builder->viewMultiple($nodes, 'teaser');.
      $build = [
        'vendor_list' => [
          '#theme' => 'item_list',
          '#items' => $vendor_list,
